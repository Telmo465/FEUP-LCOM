                    ...
                    if (msg.m_notify.interrupts & t_irq) {
                        ...
                        switch (gamestate) {
                            ...
                            case 1:
                                ...
                                draw_sprite_2(floweySprite, addrs);
                                draw_sprite(balaoFala, addrs);
                                Fala("let_s try again okay?", addrs, init_addrs, 21, t_irq, k_irq, m_irq);
                                updateBuffer(init_addrs, addrs);
                                TwoSec(t_irq, k_irq, m_irq);
                                gamestate++;
                                done = true;
...
void EasyLevel21(uint8_t *addrs, uint8_t *init_addrs, int t_irq, int k_irq, int m_irq){
    floweyState++;
    drawBackground(background1, addrs);
    Sprite *hpSprite;
    hpSprite = create_sprite(hp100, 100, 800, 10, 0);
            ...
            switch (_ENDPOINT_P(msg.m_source)) {
                case HARDWARE:
                    ...
                    if (msg.m_notify.interrupts & t_irq) {
                        ...
                        switch (gamestate) {
                            case 0:
                                Fala("is this a joke?/are you braindead?", addrs, init_addrs, 34, t_irq, k_irq, m_irq);
                                updateBuffer(init_addrs, addrs);
                                TwoSec(t_irq, k_irq, m_irq);
                                gamestate++;
                                break;
                            case 1:
                                draw_sprite(balaoFala, addrs);
                                Fala("run.", addrs, init_addrs, 4, t_irq, k_irq, m_irq);
                                updateBuffer(init_addrs, addrs);
                                TwoSec(t_irq, k_irq, m_irq);
                                gamestate++;
                                break;
                            case 2:
                                draw_sprite(balaoFala, addrs);
                                Fala("into.", addrs, init_addrs, 5, t_irq, k_irq, m_irq);
                                updateBuffer(init_addrs, addrs);
                                TwoSec(t_irq, k_irq, m_irq);
                                gamestate++;
                                break;
                            case 3:
                                draw_sprite(balaoFala, addrs);
                                Fala("the.", addrs, init_addrs, 4, t_irq, k_irq, m_irq);
                                updateBuffer(init_addrs, addrs);
                                TwoSec(t_irq, k_irq, m_irq);
                                gamestate++;
                                break;
                            case 4:
                                draw_sprite(balaoFala, addrs);
                                Fala("bone.", addrs, init_addrs, 5, t_irq, k_irq, m_irq);
                                updateBuffer(init_addrs, addrs);
                                TwoSec(t_irq, k_irq, m_irq);
                                gamestate++;
                                done = true;
...
void EasyLevel22(uint8_t *addrs, uint8_t *init_addrs, int t_irq, int k_irq, int m_irq){
    floweyState++;
    drawBackground(background1, addrs);
    Sprite *hpSprite;
    hpSprite = create_sprite(hp100, 100, 800, 10, 0);
            ...
            switch (_ENDPOINT_P(msg.m_source))
            {
                case HARDWARE:
                {
                    ...
                    if (msg.m_notify.interrupts & t_irq)
                    {
                        ...
                        if (bone1->x >= 879) {
                            ...
                            switch(floweyState){
                                case 1:
                                    ...
                                    break;
                                case 2:
                                    EasyLevel21(temp_address, init_adress, t_irq, k_irq, m_irq);
                                    break;
                                case 3:
                                    EasyLevel22(temp_address, init_adress, t_irq, k_irq, m_irq);
                                    break;
                                default:
                                    break;
                            }
                        ...
                        move_bones(bone1, temp_address, init_adress, bonescol, 1);
