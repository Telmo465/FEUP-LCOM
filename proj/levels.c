                ...
                case HARDWARE:
                {
                    ...
                    if (msg.m_notify.interrupts & t_irq)
                    {
                        ...
                        for (unsigned int x = 0; x < sizeof(bones)/sizeof(bones[0]); x++){
                            move_bones(bones[x], temp_address, init_adress, bonescol[x], 4);
                        }
                        for (unsigned int x = 0; x < sizeof(bones)/sizeof(bones[0]); x++){
            ...
            switch (_ENDPOINT_P(msg.m_source))
            {
                case HARDWARE:
                {
                    ...
                    if (msg.m_notify.interrupts & t_irq)
                    {
                        ...
                        if (sentFirst == false){
                            move_bones(bone2, temp_address, init_adress, bonescol[0], 4);
                            sentFirst = true;
                        }
                        else move_bones(bone2, temp_address, init_adress, bonescol[0], 3);
                        move_with_arrows(spr, temp_address, init_adress);
                        if (release >= 1 || passmove == true) {
                            move_bones(bone3, temp_address, init_adress, bonescol[1], 3);
                            passmove = true;
                        }
                        for (unsigned int x = 0; x < sizeof(bones)/sizeof(bones[0]); x++){
                            if(detectCollision(spr, bones[x]) && bonescol[x] == false){
            ...
            switch (_ENDPOINT_P(msg.m_source))
            {
                case HARDWARE:
                {
                    ...
                    if (msg.m_notify.interrupts & t_irq)
                    {
                        ...
                        move_bones(bone2, temp_address, init_adress, bonescol[0] , 3);
                        move_with_arrows(spr, temp_address, init_adress);
                        if (release >= 1 || passmove == true) {
                            move_bones(bone3, temp_address, init_adress, bonescol[1], 3);
                            passmove = true;
